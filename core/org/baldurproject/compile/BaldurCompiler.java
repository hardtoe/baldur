 ...
 * 
 */
public class BaldurCompiler implements Compiler {

	
	private static BaldurCompiler singleton;
	 ...
	 */
	protected ArrayList<Triple<File, JavaFile, List<JilClass>>> compilationQueue = 
		new ArrayList<Triple<File, JavaFile, List<JilClass>>>();

	protected HashMap<String, JilClass> compiledClasses =
		new HashMap<String, JilClass>();
	...
	public static BaldurCompiler get() {
		return singleton;
	}
	 ...
	 */
	public BaldurCompiler(List<String> classpath) {
	 ...
	 */
	public BaldurCompiler(
		List<String> classpath, 
		PrintStream logout
	) {
		this.loader = 
			new ClassLoader(classpath, this);
		
		this.builder = 
			new ClassFileBuilder(loader, 49);
		
		this.optimiser = 
			new BytecodeOptimiser();
	...
	public BaldurCompiler(
		...
		this.loader = 
			new ClassLoader(sourcepath, classpath, this);
	...
	public List<JilClass> compile(
		...
		SyntaxError 
	{		
		ArrayList<JilClass> classes = 
			new ArrayList<JilClass>();
		
		ArrayList<Triple<File, JavaFile, List<JilClass>>> units = 
			new ArrayList<Triple<File, JavaFile, List<JilClass>>>();
		
		for(File f : filenames) {
			Pair<JavaFile, List<JilClass>> u = innerParse(f);
			classes.addAll(u.second());
			units.add(new Triple<File, JavaFile, List<JilClass>>(f, u.first(), u.second()));			
		}
	...
	protected Pair<JavaFile,List<JilClass>> innerParse(File filename) throws IOException {				
		...
		if(parsed.contains(str_filename)) {			
			for(Triple<File,JavaFile,List<JilClass>> item : compilationQueue) {				
				if(str_filename.equals(item.first().getCanonicalPath())) {					
					return new Pair<JavaFile, List<JilClass>>(item.second(), item.third());
				}				
			}
		}
		...
		try {
			...
			compilationQueue.add(new Triple<File, JavaFile, List<JilClass>>(filename, jfile, skeletons));
			
			// finally, 
			parsed.add(str_filename);
			
			return new Pair<JavaFile, List<JilClass>>(jfile, skeletons);
     ...
     * @return
     */
	protected void finishcompilation (
		File filename, 
		JavaFile jfile,
		List<JilClass> skeletons
	) throws 
		IOException, 
		SyntaxError 
	{			
		try {			
			...
			do {
				numChanges = 
					...
					constantFolding(filename, jfile, loader);
			} while (numChanges > 0);
			...
			for(JilClass clazz : skeletons) {
				...
				isolateMethodCalls(filename, clazz, loader);		
				
			}
			
			for (JilClass c : skeletons) {
				addCompiledClass(c);
			}
			...
			// original filename.			
			String root = 
				determinePackageRoot(filename, jfile);	
			
			File outdir = 
				root == null ? 
					outputDirectory : 
					new File(outputDirectory,root);		
					
			
			for(JilClass clazz : skeletons) {				
				String baseName = 
					createBasename(clazz.type());
			...
			*/
			...
			if (se.fileName() == null) {
				throw new SyntaxError(
					se.msg(), filename.getPath(), se.line(),
					se.column(), se.width(), se);
	...
	public void addCompiledClass(JilClass c) {
		compiledClasses.put(c.type().toString(), c);
		compiledClasses.put(c.type().toString().replaceAll("<.+?>", ""), c);
	}
     ...
     * @param loader
     */
	protected int constantPropagation(
		File srcfile, 
		JavaFile jfile,
		ClassLoader loader
	) {
		...
		ConstantPropagation constantPropagation = 
			new ConstantPropagation(loader, new TypeSystem());
		
		constantPropagation.apply(jfile);
		
		logTimedMessage(
			"[" + srcfile.getPath() + "] constants propagated",
			System.currentTimeMillis() - start);
		
		return constantPropagation.getNumProps();
	}
	
	protected int constantFolding(
		File srcfile, 
		JavaFile jfile,
		ClassLoader loader
	) {
		...
		ConstantFolding constantFolding = 
			new ConstantFolding(loader, new TypeSystem());
		
		constantFolding.apply(jfile);
		
		logTimedMessage(
			"[" + srcfile.getPath() + "] constants folded",
			System.currentTimeMillis() - start);
		
		return constantFolding.getNumFolds();
	}
	...
	protected void isolateMethodCalls(
		...
		long start = System.currentTimeMillis();
		new MethodCallIsolation().apply(jfile);
		logTimedMessage("[" + srcfile.getPath() + "] Isolated methods",
				(System.currentTimeMillis() - start));
	}
	...
	public void elaborate(final String topClassName) {
		logout.println("Elaborating " + topClassName);

		try {
			Class<?> topJavaClass = 
				Class.forName(topClassName);
			
			Constructor<?> topJavaConstructor = 
				topJavaClass.getConstructor();
			
			Object topLive = 
				topJavaConstructor.newInstance();
			
			ObjectDumper dumper = 
				new ObjectDumper(this);
			
			ElabBuilder builder =
				new ElabBuilder(this);
			
			JilClass elaboratedDesign =
				builder.build(topLive);
			
			ModuleBuilder verilogBuilder =
				new ModuleBuilder(loader, this);
			
			VerilogModule module = 
				verilogBuilder.build(elaboratedDesign);
			
			System.out.println(module.toVerilog());
	...
	JilClass get(Class<?> javaClass) {
		return get(javaClass.getName());
	}

	HashSet<String> getAllFields(JilClass jilClass) {
		HashSet<String> fields = 
			new HashSet<String>();
		
		for (JilField jilField : jilClass.fields()) {
			fields.add(jilField.name());
		}

		return fields;
	}

	public JilClass get(Type.Clazz c) {
		if (c == null) {
			return null;
		} else {
	...
	JilClass get(String c) {
		return compiledClasses.get(c);
	}
