...
/**
 * The main class provides the entry point for the compiler. It is
 ...
 * 
 * Original Author: David James Pearce
 * 
 */
public class BaldurCompilerMain {
	...
	public static void main(String[] args) {		
		if(!new BaldurCompilerMain().compile(args)) {	
			System.exit(1);
		} else {
			System.exit(0);
		}
	...
	public boolean compile(String[] args) {
		...
		for (int i = 0; i != args.length; ++i) {
			if (args[i].startsWith("-")) {
				...
				if(arg.equals("-help")) {
					...
					Collections.addAll(classPath, args[++i].split(";"));
					
				} else if (arg.equals("-sourcepath")) {
					sourcePath = new ArrayList<String>();
					Collections.addAll(sourcePath, args[++i].split(";"));
					
				} else if (arg.equals("-bootclasspath")) {
					bootClassPath = new ArrayList<String>();
					Collections.addAll(bootClassPath, args[++i].split(";"));
					
				} else if (arg.equals("-d")) {
					outputDirectory = args[++i];
					
				} else if (arg.equals("-elab")) {
					elab = args[++i];
		...
		try {
			...
			BaldurCompiler compiler = 
				new BaldurCompiler(sourcePath, classPath, verbOutput);				


			compiler.setBytecodeOptimisation(true);			
	...
	public static void outputSourceError(
