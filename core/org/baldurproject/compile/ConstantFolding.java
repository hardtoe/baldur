...
import jkit.java.tree.Value.Bool;
import jkit.java.tree.Value.Int;
...
public class ConstantFolding {
	...
	public ConstantFolding(ClassLoader loader, TypeSystem types) {
		this.loader = loader; 
		this.types = types;
	}
	...
	protected Expr doBinOp(Expr.BinOp e, JavaFile file) {				
		...
		if (
			e.lhs() instanceof Value.Number &&
			e.rhs() instanceof Value.Number
		) {
			int lhs = ((Value.Int) e.lhs()).value();
			int rhs = ((Value.Int) e.rhs()).value();

			boolean blhs = (lhs == 0 ? false : true);
			boolean brhs = (rhs == 0 ? false : true);
			...
			switch (e.op()) {
				...
				case Expr.BinOp.EQ: 
					numFolds++;
					return new Value.Bool((lhs == rhs), e.attributes());
				...
				case Expr.BinOp.LAND:
					numFolds++; 
					return new Value.Bool(blhs && brhs, e.attributes());
			...
			numFolds++;
			return new Value.Int(value, e.attributes());
		}
