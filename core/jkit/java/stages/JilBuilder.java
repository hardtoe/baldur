		...
		if(tmp != null) {
			if(d.isStatic()) {				
				if(!(d.isFinal() && d.isConstant())) {
					...
					JilMethod staticInit = 
						createStaticInitialiser(parent);
					
					JilExpr.Deref df = 
						new JilExpr.Deref(
							new JilExpr.ClassVariable(parent.type()), 
							d.name(), 
							isStatic, 
							fieldT, 
							d.attributes());
				...
				for(JilMethod m : parent.methods()) {
					if(m.name().equals(parent.name())) {
						...
						JilExpr.Deref df = 
							new JilExpr.Deref(
								new JilExpr.Variable("this", parent.type()), 
								d.name(), 
								isStatic, 
								fieldT, 
								d.attributes());
