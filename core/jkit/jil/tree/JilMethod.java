...
public final class JilMethod extends SyntacticElementImpl implements jkit.compiler.Clazz.Method {
	...
	public static final class JilParameter extends SyntacticElementImpl implements Clazz.Parameter {
		...
		public void setVerilogName(final String verilogName) {
			this.verilogName = verilogName;
		}
	...
	public HashMap<String,Type> localVarTypes() {

		HashMap<String,Type> varTypes = 
		...
		for(JilStmt s : body) {
			if(s instanceof JilStmt.Assign) {
				...
				Map<String,Type> env1 = Exprs.localVariables(a.lhs());
				Map<String,Type> env2 = Exprs.localVariables(a.rhs());
				varTypes.putAll(env1);
				varTypes.putAll(env2);
		...
		for(JilParameter p : parameters) {
			varTypes.remove(p.name());
		}
		
		
		varTypes.remove("this"); // these are implicit
		varTypes.remove("super"); // these are implicit 
		
		// these are implicit
		Iterator<String> i = 
			varTypes.keySet().iterator();
	...
	public String doneSignal() {
		return getVerilogName() + "_DONE";
	}

	public String startSignal() {
		return getVerilogName() + "_START";
	}
	
	public String returnName() {
		return getVerilogName() + "_RETURN";
	}
}
